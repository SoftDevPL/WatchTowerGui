...
public class GamemodeGui extends BasicGui {
    ...
    public GamemodeGui(BasicGui previousGui) throws IllegalArgumentException {
        super(3, WatchTowerGui.getInstance().configsManager.guiLanguageConfig.getGuiLocale_gameModeGui_gameModeGuiPageName(), previousGui);
    ...
    private void setupGuiItems() {
        ...
        survival = BasicGui.createItem(Material.IRON_BLOCK, glc.getGuiLocale_gameModeGui_survival());
        creative = BasicGui.createItem(Material.GOLD_BLOCK, glc.getGuiLocale_gameModeGui_creative());
        hardcore = BasicGui.createItem(Material.MOSSY_COBBLESTONE, glc.getGuiLocale_gameModeGui_hardcore());
        spectator = BasicGui.createItem(Material.DIAMOND_BLOCK, glc.getGuiLocale_gameModeGui_spectator());
    }
    ...
    protected void init() {
        this.setItem(3, 1, survival,
                player -> player.performCommand(watchtowergui.wg.manager.CommandsManager.getMCCommand("gamemode") + " " + this.commandSurvival));
        this.setItem(4, 1, creative,
                player -> player.performCommand(watchtowergui.wg.manager.CommandsManager.getMCCommand("gamemode") + " " + this.commandCreative));
        this.setItem(5, 1, hardcore,
                player -> player.performCommand(watchtowergui.wg.manager.CommandsManager.getMCCommand("gamemode") + " " + this.commandHardcore));
        this.setItem(2, 1, spectator,
                player -> player.performCommand(watchtowergui.wg.manager.CommandsManager.getMCCommand("gamemode") + " " + this.commandSpectator));
