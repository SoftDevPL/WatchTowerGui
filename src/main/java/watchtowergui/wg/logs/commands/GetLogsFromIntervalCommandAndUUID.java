    ...
    private void changeActionForPlayerNull(CommandSender sender) {
        if (sender instanceof Player) {
            ...
            consoleChatListener.setTask(sender, (chatMessage, chatSender) -> {
                ...
                if (playerToBan == null) {
                    sendTypedMessageToSender(chatSender, languageConfig.getLogsPlayerNotExists(dates.get(0)));
                    return true;
                }
                if (dates.size() == 5) {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            ...
            if (args.length == 5) {
                ...
                if (playerToBan == null) {
                    sendTypedMessageToSender(sender, languageConfig.getLogsPlayerNotExists(args[0]));
                    return true;
                }
                Bukkit.getScheduler().runTaskAsynchronously(this.plugin,
                        () -> {
